...
class Parser(object):
    ...
    def init_private_parser(self):
        ...
        WS = self.privparser.Literal(" ") | self.privparser.Literal("\t") | self.privparser.Literal("\n")
        ...
        token = self.privparser.pyparsing_common.identifier.copy().setParseAction(
    ...
    def _compile(self, token):
        ...
        def get_token(token):
            ...
            if isinstance(tok, FutureToken):
                ...
                #print('<--', 'recursive', token, TOKENS[token])
                return TOKENS[token]
            
            if tok is not None:
                # normal token, give it away
                #print('<--', 'cache', token, tok)
                return tok
            ...
            if isinstance(k, self.pubparser.ParserElement):
                ...
                #print('    ', 'PARSEELEM', token, TOKENS[token])
                
                return TOKENS[token]
                    
            
            TOKENS[token] = FutureToken()
            #print('    ', 'toparse', token, k)
            ...
            try:
                ...
                #print('    ', 'error', token, e)
                raise
            #print('    ', 'parsedexpr', token, parsed_expr)
        ...
        WS = preparser.Literal(" ") | preparser.Literal("\t") | preparser.Literal("\n")
        ...
        token_ident = preparser.pyparsing_common.identifier.copy().setParseAction(
            lambda t: get_token(t[0])
        )
    ...
    def compile(self):
        self.tokens = {
            k: v if isinstance(v, self.pubparser.ParserElement) else self.pubparser.Forward() 
